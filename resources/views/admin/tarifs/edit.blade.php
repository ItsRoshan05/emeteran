...
<div class="max-w-2xl mx-auto bg-white p-6 rounded-xl shadow-lg">
  <h2 class="text-xl font-semibold text-[#1E3A8A] mb-4">Edit Tarif</h2>
  ...
  <form action="{{ route('admin.tarifs.update', $tarif->id) }}" method="POST">
    ...
    <div class="mb-4">
      ...
      <input type="number" name="harga_per_m3" step="0.01" min="0"
             ...
             value="{{ old('harga_per_m3', $tarif->harga_per_m3) }}" required>
      @error('harga_per_m3')
    ...
    <div class="mb-4">
      ...
      <textarea name="keterangan" rows="3"
                class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring focus:ring-blue-300">{{ old('keterangan', $tarif->keterangan) }}</textarea>
      @error('keterangan')
        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
      @enderror
    </div>
