...
<div class="bg-white rounded-xl shadow-lg p-6">
  <div class="flex justify-between items-center mb-6">
    <h2 class="text-2xl font-semibold text-[#1E3A8A]">Data Periode</h2>
  ...
  <div class="mb-4">
    <input type="text" id="customSearch" placeholder="🔍 Cari kode periode atau status..."
           class="w-full md:w-1/3 px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-600">
  </div>

  <div class="overflow-x-auto rounded-xl shadow-inner">
    <table id="tablePeriode" class="min-w-full text-sm text-gray-800 bg-white border-separate" style="border-spacing: 0">
      <thead class="bg-[#0F172A] text-white">
        <tr>
          <th class="px-5 py-3 text-left rounded-tl-xl">#</th>
          <th class="px-5 py-3 text-left">Kode Periode</th>
          <th class="px-5 py-3 text-left">Awal</th>
          <th class="px-5 py-3 text-left">Akhir</th>
      ...
      <tbody>
        ...
        <tr class="hover:bg-blue-50 border-b border-gray-200 transition duration-150">
          ...
          <td class="px-5 py-4">{{ $periode->kode_periode }}</td>
          <td class="px-5 py-4">{{ \Carbon\Carbon::parse($periode->awal)->format('d-m-Y') }}</td>
          <td class="px-5 py-4">{{ \Carbon\Carbon::parse($periode->akhir)->format('d-m-Y') }}</td>
          <td class="px-5 py-4">
            <span class="inline-block px-2 py-1 text-xs rounded-md font-semibold {{ $periode->status == 'AKTIF' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700' }}">
              {{ $periode->status }}
            </span>
          </td>
          <td class="px-5 py-4 space-x-2 whitespace-nowrap">
            ...
            <form action="{{ route('admin.periodes.destroy', $periode->id) }}" method="POST" class="inline-block"
...
<script>
  $(document).ready(function () {
    const table = $('#tablePeriode').DataTable({
      dom: 'lrtip',
      language: {
        lengthMenu: "Tampilkan _MENU_ periode",
